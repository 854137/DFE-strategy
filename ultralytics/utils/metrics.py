        ...
        if CIoU or DIoU or EIoU or SIoU or WIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                ...
                if Focal:
                    ...
                    return (getattr(WIoU_Scale, '_scaled_loss')(self),
                            (1 - iou) * torch.exp((rho2 / c2)), iou)  # WIoU https://arxiv.org/abs/2301.10051
                else:
                    return iou, torch.exp((rho2 / c2))  # WIoU v1
            if Focal:
                return iou - rho2 / c2, torch.pow(inter / (union + eps), gamma)  # Focal_DIoU
...
class ConfusionMatrix(DataExportMixin):
    ...
    def process_batch(
        ...
        conf = 0.25 if conf in {None, 0.01 if is_obb else 0.001} else conf  # apply 0.25 if default test conf is passed
    ...
    def summary(self, normalize: bool = False, decimals: int = 5) -> list[dict[str, float]]:
        ...
        Examples:
            >>> results = model.test(data="coco8.yaml", plots=True)
...
class DetMetrics(SimpleClass, DataExportMixin):
    ...
    def summary(self, normalize: bool = True, decimals: int = 5) -> list[dict[str, Any]]:
        ...
        Examples:
           >>> results = model.test(data="coco8.yaml")
...
class SegmentMetrics(DetMetrics):
    ...
    def summary(self, normalize: bool = True, decimals: int = 5) -> list[dict[str, Any]]:
        ...
        Examples:
            >>> results = model.test(data="coco8-seg.yaml")
...
class PoseMetrics(DetMetrics):
    ...
    def summary(self, normalize: bool = True, decimals: int = 5) -> list[dict[str, Any]]:
        ...
        Examples:
            >>> results = model.test(data="coco8-pose.yaml")
...
class ClassifyMetrics(SimpleClass, DataExportMixin):
    ...
    def summary(self, normalize: bool = True, decimals: int = 5) -> list[dict[str, float]]:
        ...
        Examples:
            >>> results = model.test(data="imagenet10")
