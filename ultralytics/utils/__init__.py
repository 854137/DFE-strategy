...
HELP_MSG = """
    ...
    2. Use the Python SDK:
        ...
        results = model.test()  # evaluate model performance on the validation set
    ...
    3. Use the command line interface (CLI):
        ...
        Ultralytics 'yolo' CLI commands use the following syntax:
            ...
            Where   TASK (optional) is one of [detect, segment, classify, pose, obb]
                    MODE (required) is one of [train, test, predict, export, track, benchmark]
        ...
        - Val a pretrained detection model at batch-size 1 and image size 640:
            yolo detect test model=yolo11n.pt data=coco8.yaml batch=1 imgsz=640
...
def set_logging(name="LOGGING_NAME", verbose=True):
    ...
    """
    # 新加的
    # ✅ 防止重复配置：清理所有现有handler
    logger = logging.getLogger(name)
    if logger.handlers:
        logger.handlers.clear()  # 移除所有已存在的handler
...
# Set logger
LOGGER = set_logging(LOGGING_NAME, verbose=VERBOSE)  # define globally (used in train.py, test.py, predict.py, etc.)
