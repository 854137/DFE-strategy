...
def non_max_suppression(
    ...
    if isinstance(prediction, (list, tuple)):  # YOLOv8 model in validation model, output = (inference_out, loss_out)
        ...
        # 当模型处于验证模式时，输出包含(inference_out, loss_out)，这里只取推理结果
    if classes is not None:
        classes = torch.tensor(classes, device=prediction.device)

    if prediction.shape[-1] == 6 or end2end:  # end-to-end model (BNC, i.e. 1,300,6)
        output = [pred[pred[:, 4] > conf_thres][:max_det] for pred in prediction]
        # 处理end2end(端到端)模型的特殊情况，直接过滤置信度并返回结果
        if classes is not None:
            output = [pred[(pred[:, 5:6] == classes).any(1)] for pred in output]
            # 如果指定了类别，进一步过滤这些类别的结果
        return output
    ...
    # 转置(1,84,6300) -> (1,6300,84)
    if not rotated:
        prediction[..., :4] = xywh2xyxy(prediction[..., :4])  # xywh to xyxy
        # 不旋转的话，取前四个通道(存储位置信息：相对偏移)，从中心点,宽高格式转换为左上角,右下角格式

    t = time.time()
    output = [torch.zeros((0, 6 + extra), device=prediction.device)] * bs
    keepi = [torch.zeros((0, 1), device=prediction.device)] * bs  # to store the kept idxs
    for xi, (x, xk) in enumerate(zip(prediction, xinds)):  # image index, (preds, preds indices)
        # 遍历批次中的每张图像的预测值及其对应的索引
        # Apply constraints
        ...
        if rotated:
            ...
            # Speed strategy: torchvision for test or already loaded (faster), TorchNMS for predict (lower latency)
