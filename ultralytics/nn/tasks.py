...
class BaseModel(torch.nn.Module):
    ...
    def _predict_once(self, x, profile=False, visualize=False, embed=None):
        ...
        for m in self.model:
            ...
            # 新增修改if-else，适应MobileNetV4，下面为原本代码
            # x = m(x)  # run
    ...
    def _apply(self, fn):
        ...
        if isinstance(
            m, (Detect, Detect_MultiScaleFusion,RetinaNetDetect,DetectDA,Detect_AFPN4,Detect_HSFPN)
    ...
    def loss(self, batch, preds=None):
        ...
        if getattr(self, "criterion", None) is None:
            ...
            # 获取评价尺度，这里是v8DetectionLoss()的实例化

        if preds is None:
            preds = self.forward(batch["img"])
        return self.criterion(preds, batch)
    # 这部分是对v8DetectionLoss的__call__的调用

    def init_criterion(self):
        # 这里要看子类的DetectionModel
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo11n.yaml", ch=3, nc=None, verbose=True):
        ...
        # self.model, self.save = parse_model(deepcopy(self.yaml), ch=ch, verbose=verbose)  # model, savelist
        # # 初始化自动执行parse_model
        # # parse_model返回torch.nn.Sequential(*layers)和sorted(save)
        ...
        # Build strides
        m = self.model[-1]  # Detect()
        if isinstance(m, (Detect, Detect_MultiScaleFusion,RetinaNetDetect,DetectDA,Detect_AFPN4,Detect_HSFPN,)):  # includes all Detect subclasses like Segment, Pose, OBB, YOLOEDetect, YOLOESegment
            s = 256  # 2x min stride
            # s = 640
            # MobileNetV4新增修改 s = 640
            ...
            try:
                ...
                print(
                    f"🔧 stride计算设备: {device}, 模型设备: {next(self.parameters()).device if next(self.parameters(), None) is not None else 'N/A'}")

                # 创建 fake_input 并确保在正确设备
                fake_input = torch.zeros(1, ch, s, s, device=device)

                # 确保模型在相同设备
                self.to(device)

                # 计算 stride
                outputs = _forward(fake_input)
                stride_values = [s / x.shape[-2] for x in outputs]
                m.stride = torch.tensor(stride_values, device=device)

                # ✅ 关键修复 1: 将 stride 赋值给 DetectionModel 对象
                self.stride = m.stride.clone()
                print(f"✅ stride 计算成功: {self.stride}, device: {self.stride.device}")

            except Exception as e:
                print(f"❌ stride 计算失败: {e}")
                # 回退到默认值
                device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
                default_stride = torch.tensor([8, 16, 32], device=device)

                # ✅ 关键修复 2: 无论如何都要确保 DetectionModel 有 stride 属性
                self.stride = default_stride.clone()
                m.stride = default_stride.clone()
                print(f"🔧 使用默认 stride: {self.stride}")
    ...
    def init_criterion(self):
        ...
        # 一般不是端到端，返回v8DetectionLoss(self)
        return E2EDetectLoss(self) if getattr(self, "end2end", False) else v8DetectionLoss(self)
    # 这里的self将DetectionModel中所有self命名的变量都传进去v8DetectionLoss进行初始化（实例化）了
    # 在这里self.model被传进去了，
    # return v8DetectionLoss(self) 即return v8DetectionLoss(model=model,...)
    # 也可以写成return v8DetectionLoss(DetectionModel(model=model,...))
...
def parse_model(d, ch, verbose=True):
    ...
    # 确保YAML中的变量名能解析，新加
    nc = d.get('nc', 80)
    num_anchors = d.get('num_anchors', 9)
    reg_max = d.get('reg_max', 4)
    ...
    # 从字典d（yaml配置文件中的键值对）中提取出模型的基本配置，如类别数nc，激活函数act和不同的模型尺寸scales
    depth, width, kpt_shape = (d.get(x, 1.0) for x in ("depth_multiple", "width_multiple", "kpt_shape"))
    scale = d.get("scale")
    if scales:
    # 若字典d存在scales，则提供字典d中的键scale（与scales区分）对应的值
        if not scale:
        # 若scale（不是scales）不存在
            scale = tuple(scales.keys())[0]
            # scale取scales中键（keys）的第0个键
    ...
    if act:
        ...
        # 使用eval函数执行act字符串中的Python表达式。动态评估act中的内容并返回结果。
        # 例如，act是“nn.ReLU()”，那么eval(act)将返回一个ReLU激活函数的实例。
        # 然后，这个结果被赋值给Conv.default_act, 从而重新定义Conv类的激活函数。
        if verbose:
    ...
    ch = [ch]
    # ch为上面各层的输出
    #DetectionModel中默认ch=3，作为输入
    layers, save, c2 = [], [], ch[-1]  # layers, savelist, ch out
    # 初始化layers, save, c2
    # ch的最后一个元素为上一层的输出, c2为下一层的输入
    base_modules = frozenset(
        {
            ...
            # Bag,
            # 该模块需要三个输入的通道数和空间大小都一致才行！
            # [4, 6, 9], 1, Bag, [channel_out]
            # Light_Bag,
            # PAPPM
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        # for 用来得到所有层数的神经网络，若为第一层的CBS网络：[f, n, m, args] = [-1, 1, Conv, [64, 3, 2]]
        # 遍历yaml中的backbone和head
        # 新增修改，适应MobileNetV4
        t = m
        ...
        # 先判断if的条件，如果m字符串中有nn.的话，执行m = getattr(torch.nn, m[3:]，此时m变为calss类，即m==Conv类
        # m == "<class'__main__.Conv'>"
        # 否则执行globals()[m]
        ...
        if m in base_modules:
            c1, c2 = ch[f], args[0]
            # f=-1，args=[64, 3, 2],
            # 则c1输入为上一层输出,c2为yaml中定义
            if c2 != nc:  # if c2 not equal to number of classes (i.e. for Classify() output)
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            # 确认是否为最终分类头，不是的话确保输出通道缩放正确
            # 遇见64分类的则会出现BUG
            ...
            args = [c1, c2, *args[1:]]
            # 更新args[64, 3, 2]为[3, 64, 3, 2],作为m模块的初始化参数
            if m in repeat_modules:
                args.insert(2, n)  # number of repeats
                n = 1

            if m in {DSC3k2}:  # for M/L/X sizes
                legacy = False
                if scale in "mlx":
                    args[3] = True
        ...
        elif m is ResNetLayer_1834:
            # ResNetLayer_1834 是基础块 (Basic Block), 无通道扩展 (适用于 ResNet18/34)
            # args[1] 就是最终的输出通道数
            c2 = args[1]
            ...
            {Detect, WorldDetect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB, ImagePoolingAttn, v10Detect,Detect_MultiScaleFusion,RetinaNetDetect,DetectDA,Detect_AFPN4,Detect_HSFPN}
        ):
            args.append([ch[x] for x in f])
            # args：[80]
            # 此时f为[15, 18, 21]
            # ch为从第0层到第21层的输出通道数
            # 执行完之后：args=[80, ch[15], ch[18], ch[21]]
        ...
        #     # args = [1024*width, 7]
        # # 自加
        # elif m is Custom_CBAM:
        #     c1 = ch[f]
        #     c2 = c1
        #     args = [c1, *args]
        # elif m is DCNv3_Local:
        #     c1 = ch[f]
        #     c2 = c1
        #     args = [c1, *args]

        elif m in onlyincha_modules:
            c1 = ch[f]
            c2 = c1
            # 不需要：
            #  c2 = args[0]
            #  if c2 != nc:  # if c2 not equal to number of classes (i.e. for Classify() output)
            #  c2 = make_divisible(min(c2, max_channels) * width, 8)
            #  这是因为c1本身就是缩放过的，这里c2不能再缩放
            #  1、所以原类（方法）初始化需要输入输出通道数的，
            #  2、并且在netyaml中有传入的输出通道数c2（args[0]）
            #  才需要缩放
            args = [c1, *args]

        elif m is SDP_Local:# SDP
            c1 = ch[f[-1]]
            c2 = c1
            # 保证输出特征通道数等于低层特征通道数，而不是高层通道数
            args = [c1, *args]
            # 这样输入通道数就不用自己设置了

        # elif m is Pag:
        #     # [-1, 6], 1, Pag, [False, False]
        #     # 该模块需确保两个输入空间大小一致！
        #     # 模块用在FPN的Concat之后，-1对应的通道数多的，6对应骨干网络中通道数少的
        #     c1 = ch[f[1]]
        #     c2 = ch[f[0]]
        #     # 保证输出特征通道数等于低层特征通道数，而不是高层通道数
        #     args = [c2, c1, *args]
        ...
        # elif m is InternImageBlock:
        #     # [-1, 12, InternImageBlock, [8, False]]
        #     c1 = ch[f]
        #     c2 = c1
        #     args = [c1, *args]
        #     args.insert(2, n)
        ...
        #     args = [c1, c2, *args[1:]]
            ...
            # 为了使Upsample正常使用
            c2 = ch[f]
            # 改变c2是为了将上采样当作一层网络，记录输出通道数到ch列表
            # c1本身就是缩放过的，这里c2不能再缩放
        elif m is torch.nn.MaxPool2d:
            c2 = ch[f]
            # 其他参数不用在这里更新

        elif m is DownsampleLayer:
            c1 = ch[f]
            c2 = 2 * c1
            args = [c1, *args]

        elif m in (MultiScaleAttentionFusion_TB, MultiScaleAttentionFusion_SA, MultiScaleAttentionFusion_SpA):
            # 更新实际传入的数据
            # args存着原始输入;列表的下标不能是列表
            c1_0 = ch[f[0]]
            c1_1 = ch[f[1]]
            c1_2 = ch[f[2]]
            if c2 != nc:  # if c2 not equal to number of classes (i.e. for Classify() output)
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [[c1_0, c1_1, c1_2],c2, *args[1:]]
            ...
            c2 = ch[f]
        # 新增修改if-else，适应MobileNetV4。下面为改动前
        ...
        save.extend(
            x % (i + 4 if backbone else i) for x in ([f] if isinstance(f, int) else f) if x != -1)  # append to savelis
        layers.append(m_)
        if i == 0:
            ch = []
        if isinstance(c2, list):
...
def yaml_model_load(path):
    ...
    unified_path = re.sub(r"(\d+)([nslmx])(.+)?$", r"\1\3", str(path))  # i.e. yolov8x.yaml -> yolov8_Patent1.yaml
    ...
    if isinstance(model, torch.nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, (Segment, YOLOESegment)):
                ...
                return "obb"
            elif isinstance(m, (Detect, WorldDetect, YOLOEDetect, v10Detect,Detect_MultiScaleFusion,RetinaNetDetect,DetectDA,Detect_AFPN4,Detect_HSFPN)):
                return "detect"
